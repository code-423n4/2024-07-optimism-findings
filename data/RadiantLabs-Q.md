## [QA-01] MIPS: The MIPS VM do not throw on overflow exceptions for signed arithmetic `add`, `addi`, `sub` instructions which deviates from MIPS spec.
...
## [QA-02] MIPS: The MIPS VM do not throw when unaligned effective memory address is used for certain instructions

As per the MIPS spec in [link](https://www.cs.cmu.edu/afs/cs/academic/class/15740-f97/public/doc/mips-isa.pdf), certain instructions are supposed to throw if the effective memory address M[R[rs]+SignExtImm] is unaligned, but they currently don't.
...
```
There are many instructions that throw an exception on unaligned memory, search `AddressError` in the MIPS document in [link](https://www.cs.cmu.edu/afs/cs/academic/class/15740-f97/public/doc/mips-isa.pdf) for more details. The only impact of this is that it deviates from the MIPS specification as the offchain MIPS VM has the same bug. 
...
`state.step` is incremented every MIPS instruction executed, it is a `uint64` variable so it can overflow if there are more than `2^64` MIPS instructions when `MAX_DEPTH - SPLIT_DEPTH - 2 = 64`
...
This is also non-exploitable for the same reasons as above issue.

## [QA-12] PREIMAGE: Wrong comparison operator in `relativeOffset` may cause unintentional reverts

The case `relativeOffset + 32 == _input.length` is fine because only calldata up to `relativeOffset + 31` will be loaded. It can cause unintentional reverts in specific values of `partOffset`
```solidity
...
For both `loadKeccak256PreimagePart` and `loadSha256PreimagePart`, it is better to use `.length` instead of using `calldataload` at a fixed offset to determine the length of calldata because calldata offsets can be manipulated although it cannot in this case.
```solidity
...
The absorption + permutation are superfluous because that is what the state commitments contain, if you check the `squeeze` function we never use the `_postState.stateCommitment` and the `_stateMatrix` is the one at the `_preState`

So instead the function could just take in the `_stateMatrix` after absorption of the `_postState`, check it against` _postState.stateCommitment` and `squeeze` it
```solidity
...
## [QA-19] PREIMAGE: If a keccak256 preimage is larger than max size of LPP, a valid state consisting it cannot be proven

We don't know how large a keccak256 preimage can be, but if its greater than the max size of LPP, it cannot be added to the oracle via the `loadKeccak256PreimagePart` as well due to exceeding block gas limits and a valid state potentially cannot be proven.
...
## [QA-21] FDG: LPP cost may not cover bonds at `MAX_DEPTH` when gas is expensive.

The bond for a `MAX_DEPTH` claim is fixed at `300_000_000 * 200 Gwei = 60 ETH` which is supposed to be double the estimated cost of an passing a maximum size LPP. However, the gas price is actually dynamic and dependent on factors such as network demand for gas (EIP-1559). If the gas exceeds the fixed cost 200 gwei * 2 = 400 gwei, then the total cost for passing a maximum size LPP will exceed the bond reimbursed to the user leading to less incentives to pass the LPP. An oracle should be used instead to determine the gas price instead of hardcoded gas prices.
